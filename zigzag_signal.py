...
def run():
    ...
    # Ha semmi nem talalt, debug
    if not all_found_patterns:
        x = recent[-5][1] if len(recent) >= 5 else pivots[-5][1]
        a = recent[-4][1] if len(recent) >= 4 else pivots[-4][1]
        b = recent[-3][1]
        c = recent[-2][1]
        d = recent[-1][1]
        ratios = calc_ratios(x, a, b, c, d)
        ...
        if ratios:
            xab, abc, bcd, xad = ratios
            ratio_str = f"XAB={xab:.3f} ABC={abc:.3f} BCD={bcd:.3f} XAD={xad:.3f}"
        debug_msg = f"""🔍 <b>DEBUG - Nincs minta</b>
XABCD: X={x:.2f} A={a:.2f} B={b:.2f} C={c:.2f} D={d:.2f}
Ar: ${current_price:,.2f}
{ratio_str}
ZigZag pontok: {len(pivots)}"""
        send_telegram(debug_msg)
        print("Nincs minta - debug uzenet kuldve")
        return
